            ...
            <div class="row justify-content-between align-items-center">
                ...
                <div class="col-5">
                    <div class="right_btn_div d-flex ml-auto justify-content-end">
                        <div class="white_btn d-flex justify-content-between align-items-center">
                            <p class="mb-0 ">SMS Credits: <span class="h3">$<span v-if="parseFloat(charge.balance_remaining) > 0">{{parseFloat(charge.balance_remaining).toFixed(2)}}</span><span v-else>0.00</span></span></p>
                        </div>
            ...
            <div class="row">
                <div class="col-4">
                    <div class="box_div box_shadow_reg border-radius-reg">
                        ...
                        <div class="prod_numer_div text-center">
                            <div class="price_div display-1 font-400 text-blue">{{shop.currency_symbol}}{{analytics.revenue.toFixed(2)}}</div>
                        </div>
                        <div class="link_div">
                            <p class="text-center mb-0">Sales</p>
                        </div>
                    </div>
                </div>
                <div class="col-4">
                    <div class="box_div box_shadow_reg border-radius-reg">
                        ...
                        <div class="prod_numer_div">
                            <div class="price_div display-1 font-400 text-center text-blue">{{analytics.abandoned_orders}}</div>
                        </div>
                        <div class="link_div">
                ...
                <div class="col-4">
                    <div class="box_div box_shadow_reg border-radius-reg">
                        ...
                        <div class="prod_numer_div text-center">
                            <div class="price_div display-1 font-400 text-blue">{{analytics.orders}}</div>
                        </div>
                        <div class="link_div">
                            <p class="text-center mb-0">Quantity</p>
                        </div>
        ...
        <div class="table_div sms-automation-table">
            <div class="table_heading">
                <div class="row align-items-center">
                    <div class="col-6">
                        <p class="title_p display-3 font-400 mb-0">Letest Messages</p>
                    </div>
                    <div class="col-6 text-right">
                        <button class="btn_all" @click="changeAction('sms/addreminder')">+ Add Reminder</button>
                    </div>
                </div>
            </div>
            ...
            <div class="table_div_inner box_shadow_reg border-radius-reg">
                <table class="table mb-0">
                    ...
                    <tr v-for="(automation, index) in automations.data">
                        ...
                        <td>{{(automation.total_clicked == 0) ? '-' : ((automation.total_clicked / automation.total_sent) * 100).toFixed(1) + '%'}}</td>
                        <td>{{automation.total_order}}</td>
                        <td>{{(automation.total_order == 0) ? '-' : ((automation.total_order / automation.total_sent) * 100).toFixed(1) + '%'}}</td>
                        <td>{{automation.total_revenue}}</td>
                        <td>${{parseFloat(automation.cost).toFixed(2)}}</td>
                        <td>{{(automation.total_revenue == 0) ? '-' : ((automation.total_revenue / automation.cost) * 100).toFixed(1) + '%'}}</td>
                        <td>
                            <input type="checkbox" class="checkbox" v-model="automation.is_active" @change="updateStatus(automation.id)"/>
                        </td>
                        <td>
                            <button class="btn_table btn_blue font-400" @click="changeAction('sms/editreminder/' + automation.id)">Edit</button>
...
export default {
    ...
    methods:{
        ...
        async getData(url = 'get-automations/sms'){
            let base = this;
            base.isLoading = true;
            await axios.get(url)
                .then(res => {
                    ...
                    base.analytics = data.analytics.sms;
                    base.shop = data.shop;
                    base.charge = res.data.data.charge;
                })
                .catch(err => {
                    console.log(err);
                })
