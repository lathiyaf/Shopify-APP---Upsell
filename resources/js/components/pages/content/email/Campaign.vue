<template>
    <div class="email-campaigns" v-if="!isLoading">
        <div class="row_head_time">
            <div class="row justify-content-between align-items-center">
                <div class="col-5">
                    <div class="normal_content">
                        <p class="mb-0 font-400">Automate your sms abandoned cart sequence here!,<br>
                            *Pro-tip: Incentivize them in your last sms to encourage checkout!</p>
                    </div>
                </div>
                <div class="col-5">
        ...
        <div class="revanu_div sms-automation">
            <div class="row">
                <div class="col-4">
                    <div class="box_div box_shadow_reg border-radius-reg">
                        ...
                        <div class="prod_numer_div text-center">
                            <div class="price_div display-1 font-400 text-blue">$0.00</div>
                        </div>
                        <div class="link_div">
                ...
                <div class="col-4">
                    <div class="box_div box_shadow_reg border-radius-reg">
                        ...
                        <div class="prod_numer_div">
                            <div class="price_div display-1 font-400 text-center text-blue">0</div>
                        </div>
                        <div class="link_div">
                            <p class="text-center mb-0">Quantity</p>
                        </div>
                ...
                <div class="col-4">
                    <div class="box_div box_shadow_reg border-radius-reg">
                        ...
                        <div class="prod_numer_div text-center">
                            <div class="price_div display-1 font-400 text-blue">1</div>
                        </div>
                        <div class="link_div">
                            <p class="text-center mb-0">Quantity</p>
                        </div>
        ...
        <div class="table_div sms-automation-table">
            <div class="campain_head">
                <p class="display-1 font-400 mb-2">Campaigns</p>
                <p class="font-400">Send once off campaigns for new products, announcements and any updates that your visitors may be interested in!
                    <br>*Pro-tip: Add in a special thank you discount to sweeten the deal!</p>
            </div>
            ...
            <div class="table_heading">
                <div class="row align-items-center">
                    ...
                    <div class="col-6 text-right">
                        <button class="btn_all" @click="changeAction('email/addcampaign')">+ Create New Campaign</button>
                    </div>
                </div>
            </div>

            <div class="table_div_inner box_shadow_reg border-radius-reg" v-if="campaigns.data.length">
                <table class="table mb-0">
                    ...
                    <tr v-for="(campaign, index) in campaigns.data">
                        <td>{{index + 1}}</td>
                        <td>{{campaign.body_text.subject}}</td>
                        <td>{{campaign.created_at  }}</td>
                        <td>
                            <button class="btn_table btn_blue font-400" @click="changeAction('email/editcampaign/' + campaign.id)">Edit</button>
                            <button class=" ml-1 btn_table btn_red font-400" @click="deleteAutomation(campaign.id)">Delete</button>
                        </td>
                    </tr>
                    </tbody>
                </table>
            </div>
            <div class="capmaine_footer mt-5" v-else>
                <div class="row align-items-center ">
                    <div class="col-md-6">
                        <p class="font-400 ">Draft will appear here</p>
                    </div>
                    <div class="col-md-6">
                        <p class="font-600 display-3 text-gray text-right">You have no <span class="text-dark">Drafts</span></p>
                    </div>
                </div>
            </div>
<!--            sent campagin table-->
            ...
            <div class="table_heading">
                <div class="row align-items-center">
                    <div class="col-6">
                        <p class="title_p display-3 font-400 mb-0">History</p>
                    </div>
                </div>
            </div>
            <div class="table_div_inner box_shadow_reg border-radius-reg" v-if="sentCampaigns.data.length">
                <table class="table mb-0">
                    <thead class="thead-blue">
                    <tr>
                        <th>Text</th>
                    ...
                    <tbody>
                    <tr v-for="(campaign, index) in sentCampaigns.data">
                        <td>{discountValue}% OFF at {siteName}, discount appli...</td>
                        <td>{{campaign.created_at}}</td>
                        <td>Completed</td>
                        <td>{{campaign.is_sent}}</td>
                        <td>{{campaign.is_clicked}}</td>
                        <td>{{(campaign.ctr == 0) ? '-' : campaign.ctr + '%'}}</td>
                        <td>0</td>
                        <td>{{(campaign.cvr == 0) ? '-' : campaign.cvr + '%'}}</td>
            ...
            <div class="capmaine_footer mt-5" v-else>
                <div class="row align-items-center ">
                    <div class="col-md-6">
                        <p class="font-400 ">Your Campaign history will appear here</p>
                    </div>
                    <div class="col-md-6">
                        <p class="font-600 display-3 text-gray text-right">You have no <span class="text-dark">History</span></p>
                    </div>
                </div>
            </div>
        </div>
        <pagination :data="campaigns" @update="changePageData"></pagination>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        async getData(url = 'get-automations/email/campaign'){
            let base = this;
            base.isLoading = true;
            await axios.get(url)
                .then(res => {
                    let data = res.data.data;
                    base.campaigns = data.automations;
                    base.sentCampaigns = data.sent;
                })
                .catch(err => {
                    console.log(err);
                })
